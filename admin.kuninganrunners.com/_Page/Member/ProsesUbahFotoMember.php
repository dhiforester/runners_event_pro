<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Validasi id_member
        if (empty($id_member)) {
            $response['message'] = 'ID Member tidak ditemukan pada database.';
        ...
        if (!isset($_FILES['foto']) || $_FILES['foto']['error'] !== UPLOAD_ERR_OK) {
            $response['message'] = 'File foto belum dipilih atau terjadi kesalahan saat upload.';
            echo json_encode($response);
            exit;
        }
        ...
        $uploadDir = '../../assets/img/Member/'; // Folder tujuan upload
        ...
        $queryOldFile = "SELECT foto FROM member WHERE id_member = ?";
        $stmtOldFile = $Conn->prepare($queryOldFile);
        $stmtOldFile->bind_param('s', $id_member);
        $stmtOldFile->execute();
