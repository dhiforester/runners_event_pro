<?php
    ...
    if(empty($SessionIdAkses)){
        ...
        if(!empty($_POST['id_web_log'])){
            $CheckLogItem=$_POST['id_web_log'];
            $JumlahDipilih=count($CheckLogItem);
            $JumlahBerhasil=0;
            foreach($CheckLogItem as $List){
                $HapusLog = mysqli_query($Conn, "DELETE FROM web_log WHERE id_web_log='$List'") or die(mysqli_error($Conn));
                if($HapusLog){
            ...
            if($JumlahDipilih==$JumlahBerhasil){
                ...
                $kategori_log="Log Viewer";
                $deskripsi_log="Hapus Log Viewer";
