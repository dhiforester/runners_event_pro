<?php
    ...
    if($ValidasiKelengkapanData!=="Valid"){
        ...
        if( $validasi_varian!=="Valid"){
            ...
            $jumlah=$qty*$harga;
            // Cek Apakah Ada Item Barang Ini Untuk kode_transaksi Ini dan Barang Ini Sudah Ada
            $QryParam = mysqli_query($Conn,"SELECT id_transaksi_rincian, qty FROM transaksi_rincian WHERE id_barang='$id_barang' AND kode_transaksi='$kode_transaksi' AND varian like '%$id_varian%'")or die(mysqli_error($Conn));
            $DataParam = mysqli_fetch_array($QryParam);
            if(empty($DataParam['id_transaksi_rincian'])){
                ...
                $stmt->bind_param("ssssssss", $kode_transaksi, $id_member, $id_barang, $nama_barang, $varian_json, $harga, $qty, $jumlah);
                if ($stmt->execute()) {
                    $validasi_proses_rincian="Valid";
                }else{
                    $validasi_proses_rincian= 'Terjadi kesalahan pada saat menambah item pada rincian transaksi';
                }
            }else{
                $id_transaksi_rincian=$DataParam['id_transaksi_rincian'];
            ...
            if($validasi_proses_rincian!=="Valid"){
                ...
                if ($stmtUpdate->execute()) {
                    ...
                    $deskripsi_log="Tambah Rincian Transaksi";
                    $InputLog=addLog($Conn,$SessionIdAkses,$now,$kategori_log,$deskripsi_log);
                    if($InputLog=="Success"){
                        $response['success'] = true;
                        $response['message'] = 'Tambah Rincian Berhasil';
                    }else{
