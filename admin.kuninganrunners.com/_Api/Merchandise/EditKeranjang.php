            ...
            if (!isset($headers['x-token'])) {
                ...
                if (!isset($Tangkap['email'])) {
                    ...
                    if (!isset($Tangkap['id_member_login'])) {
                        ...
                        // Validasi id_transaksi_keranjang tidak boleh kosong
                        if (!isset($Tangkap['id_transaksi_keranjang'])) {
                            ...
                            $id_transaksi_keranjang = validateAndSanitizeInput($Tangkap['id_transaksi_keranjang']);
                                        ...
                                        if($datetime_expired<$now){
                                            ...
                                            if($email_member!==$email){
                                                ...
                                                if(!empty($varian)){
                                                    ...
                                                    if(!empty(count($varian_array))){
                                                        ...
                                                        if(empty($id_varian)){
                                                            ...
                                                            $validasi_varian="Item Barang Telah Berubah, Anda Harus Memilih Diantara Varian Yang Ada. Sistem Kami Telah Menghapus Item Keranjang Ini.";
                                                        }else{
                                                ...
                                                if( $validasi_varian!=="Valid"){
                                                    ...
                                                    // Apabila QTY Diisi Koaong Maka Hapus
                                                    if(empty($qty)){
                                                        $HapusKeranjang = mysqli_query($Conn, "DELETE FROM transaksi_keranjang WHERE id_transaksi_keranjang='$id_transaksi_keranjang'") or die(mysqli_error($Conn));
                                                        if($HapusKeranjang){
                                                            $ValidasiProses = "Valid";
                                                        }else{
                                                            $ValidasiProses = "Terjadi kesalahan saat menghapus item keranjang";
                                                        }
                                                    }else{
                                                        //Apabila Ada Maka Update Keranjang
