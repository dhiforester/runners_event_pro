<?php
    ...
    $service_name = "Pembatalan Pendaftaran Event";
            ...
            if (!isset($headers['x-token'])) {
                ...
                if (!isset($Tangkap['email'])) {
                    ...
                    if (!isset($Tangkap['id_member_login'])) {
                        ...
                        if (!isset($Tangkap['id_event_peserta'])) {
                                    ...
                                    if (!empty($DataMember['id_member'])) {
                                        ...
                                        if($datetime_expired<$now){
                                            ...
                                            if($email_member!==$email){
                                                ...
                                                // Cek Apakah ID Peserta Sesuai
                                                $ValidasiPesertaEvent = mysqli_num_rows(mysqli_query($Conn, "SELECT id_event_peserta FROM event_peserta WHERE id_event_peserta='$id_event_peserta' AND id_member='$id_member'"));
                                                if(empty($ValidasiPesertaEvent)){
                                                    $keterangan = "ID Peserta Event Yang Anda Gunakan Tidak Valid";
                                                }else{
                                                    //Buka Data Transaksi
                                                    $status = GetDetailData($Conn, 'event_peserta', 'id_event_peserta', $id_event_peserta, 'status');
                                                    //Apabila Status Lunas Maka Tidak Bisa
                                                    if($status!=="Pending"){
                                                        $keterangan = "Pendaftaran Event Tidak Bisa Dibatalkan";
                                                    }else{
                                                        //Hapus Event Peserta
                                                        $HapusEventPeserta = mysqli_query($Conn, "DELETE FROM event_peserta WHERE id_event_peserta='$id_event_peserta'") or die(mysqli_error($Conn));
                                                        if($HapusEventPeserta){
                                                            //Apabila Berhasil Lanjutkan Menghapus Transaksi Jika Ada
                                                            ...
                                                            if($HapusTransaksi){
                                                                ...
                                                                $keterangan = "Terjadi kesalahan pada saat menghapus data transaksi";
                                                            }
                                                        }else{
                                                            $keterangan = "Terjadi kesalahan pada saat menghapus data peserta event";
                                                        }
                                                    }
                                                }
                                            }
