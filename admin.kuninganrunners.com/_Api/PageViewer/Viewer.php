            ...
            if (!isset($headers['x-token'])) {
                ...
                if ($DataValidasiToken) {
                    ...
                    if ($now >= $datetime_creat && $now <= $datetime_expired) {
                        ...
                        //Menghitung Data
                        $this_month=date('Y-m');
                        $hit_page = mysqli_num_rows(mysqli_query($Conn, "SELECT id_web_log FROM web_log"));
                        $hit_page_month = mysqli_num_rows(mysqli_query($Conn, "SELECT id_web_log FROM web_log WHERE tanggal like '%$this_month%'"));
                        $unique_page = mysqli_num_rows(mysqli_query($Conn, "SELECT DISTINCT ip_viewer FROM web_log"));
                        $unique_page_month = mysqli_num_rows(mysqli_query($Conn, "SELECT DISTINCT ip_viewer FROM web_log WHERE tanggal like '%$this_month%'"));
                        
                        $metadata=[
                            "hit_page" => $hit_page,
                            "hit_page_month" => $hit_page_month,
                            "unique_page" => $unique_page,
                            "unique_page_month" => $unique_page_month
                        ];
                        //menyimpan Log
