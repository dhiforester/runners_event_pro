<?php
	...
	if(empty($Tangkap['merchant_id'])){
		$Array = Array (
			"pesan" => "Marchant ID Tidak Boleh Kosong",
			"code" =>201
		);
	}else{
		if(empty($Tangkap['transaction_time'])){
			$Array = Array (
				"pesan" => "Transaction time Tidak Boleh Kosong",
				"code" =>201
			);
		}else{
			$merchant_id=$Tangkap['merchant_id'];
			$transaction_time=$Tangkap['transaction_time'];
		    //Buka Pengaturan
		    $QryPayment = "SELECT * FROM log_payment  WHERE merchant_id='$merchant_id' AND transaction_time like '%$transaction_time%'";
